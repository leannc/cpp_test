...
// 生成随机数填充矩阵
void fill_matrix(std::vector<std::vector<double>> &matrix, uint64_t row,
                 uint64_t col) {  // NOLINT
  std::mt19937 gen(42);
...
void BM_VectorFlat_double_parallel(benchmark::State &state) {  // NOLINT
  ...
  for (auto _ : state) {
    std::for_each(std::execution::par, matrix.begin(), matrix.end(),
                  [&sum](const int &element) { sum += element; });
  }
...
void BM_VectorFlat_double_SIMD(benchmark::State &state) {  // NOLINT
  ...
  for (auto _ : state) {
    ...
    for (; i + 4 < total; i += 4) {
      __m256d data =
          _mm256_loadu_pd(&matrix[i]);         // 加载 4 个 double 元素到寄存器
    ...
    sum = partial_sums[0] + partial_sums[1] + partial_sums[2] +
          partial_sums[3];  // cppcheck-suppress unreadVariable
