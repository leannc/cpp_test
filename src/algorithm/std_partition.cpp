...
void std_partition()
{
    ...
    std::partition_copy(std::begin(v),std::end(v),std::back_inserter(even),std::back_inserter(odd),[](int i){return i%2 ==0;});

    std::cout<<"even number:"<<std::endl;
    for(auto n:even) std::cout<<n<<std::endl;

    std::cout<<"odd number:"<<std::endl;
    for(auto n:odd) std::cout<<n<<std::endl;
