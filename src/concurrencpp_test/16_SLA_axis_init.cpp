...
result<int> axis_move_task(std::shared_ptr<concurrencpp::thread_pool_executor> tpe,
  ...
  if (result == -1) {
    ...
    auto guard = co_await _lock.lock(tpe);
    std::cout << currentTime() << axis_name << ": previous task done!  begin to move task "
...
result<int> start_task() { co_return -1; }

int SLA_axis_init() {
  runtime runtime;
  const auto thread_pool_executor = runtime.thread_pool_executor();

  concurrencpp::timer timer = runtime.timer_queue()->make_timer(3s, 3s, thread_pool_executor, [&] { i = 100; });
  ...
  // 刮刀轴回零
  recoater_platform_init_task = axis_go_origin_task(thread_pool_executor, runtime.timer_queue(),
                                                    std::move(recoater_platform_init_task), "recoater");
  // 网板回零
  recoater_platform_init_task = axis_go_origin_task(thread_pool_executor, runtime.timer_queue(),
                                                    std::move(recoater_platform_init_task), "platform");
  // 网板运动到指定位置
  recoater_platform_init_task = axis_move_task(thread_pool_executor, runtime.timer_queue(),
                                               std::move(recoater_platform_init_task), 10, "platform");

  // 液位轴回零
  resin_init_task =
      axis_go_origin_task(thread_pool_executor, runtime.timer_queue(), std::move(resin_init_task), "resin");
  // 液位轴运动到指定位置
  resin_init_task =
      axis_move_task(thread_pool_executor, runtime.timer_queue(), std::move(resin_init_task), 10, "resin");
