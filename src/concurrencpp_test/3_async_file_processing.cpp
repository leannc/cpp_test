...
replace_chars_in_file(std::shared_ptr<concurrencpp::thread_pool_executor> background_executor,
  ...
  auto processed_file_content =
      co_await threadpool_executor->submit([file_content = std::move(file_content), from, to]() mutable {
...
int async_file_processing() {
  ...
  try {
    replace_chars_in_file(runtime.background_executor(), runtime.thread_pool_executor(), file_path, from_char, to_char)
        .get();
