...
/*
        ...
        The original version of this application does this rescheduling implicitly, by awaiting results that
   executor::submit produces. this version explicitly switches execution between executors and does not create sub-tasks
   from lambdas. Both versions are identical in terms of functionality and the final outcome.
*/

#include "UseCase.h"
concurrencpp::result<void>
replace_chars_in_file_version_2(std::shared_ptr<concurrencpp::thread_pool_executor> background_executor,
                                std::shared_ptr<concurrencpp::thread_pool_executor> threadpool_executor,
                                const std::string file_path, char from, char to) {
