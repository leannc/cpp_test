...
//
/*
    In this example we will collect all prime numbers from 0 to 1,000,000 in a parallel manner, using parallel
   coroutines.
 */
...
// runs in parallel
concurrencpp::result<std::vector<int>>
...
concurrencpp::result<std::vector<int>> find_prime_numbers(std::shared_ptr<concurrencpp::executor> executor) {
  ...
  const auto range_length = 1000 / concurrency_level;
