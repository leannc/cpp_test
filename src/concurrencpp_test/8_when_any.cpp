...
int example_job_any(int task_num, int dummy_value, int sleeping_time_ms) {
  const auto msg0 = std::string("task num: ") + std::to_string(task_num) + " is going to sleep for " +
                    std::to_string(sleeping_time_ms) + " ms";
...
consume_tasks_as_they_finish(std::shared_ptr<concurrencpp::thread_pool_executor> resume_executor,
  ...
  while (!results.empty()) {
    ...
    auto msg = std::string("task num: ") + std::to_string(when_any.index) +
               " is finished with value of: " + std::to_string(done_value);
...
int concurrencpp_when_any() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    results.emplace_back(background_executor->submit(example_job_any, i, i * 15, sleeping_time_ms));
  }
