...
public:
    StealConstructionParam(
        ...
        const std::string& const_reference_,
        std::string&& lvalue_to_rvalue_reference_,
        const std::string& rvalue_to_const_reference_  ///这个地方，如果想要外面直接传rvalue，那就必须是用const &,否则编译无法通过。
        )
    : value(std::move(value_))
    , reference(std::move(reference_))
    , const_reference_ref(std::move(const_reference_))
...
void std_move()
{
    ...
    StealConstructionParam stealer(value,reference,const_reference,std::move(lvalue_to_rvalue_reference),"rvalue valid");
