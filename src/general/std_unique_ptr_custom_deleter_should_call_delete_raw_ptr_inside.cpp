...
class MyClass {  // NOTE: 在匿名空间中的类,只有这cpp个文件看得见,就不会跟别的文件打搅
 public:
  explicit MyClass(std::string&& name):name_(std::move(name)) {std::cout<< name_ <<" is constructed" << std::endl;}
...
void std_unique_ptr_custom_deleter_should_call_delete_raw_ptr_inside() {
  ...
  std::unique_ptr<MyClass, decltype(no_delete_deleter)> c2(new MyClass("c2"), no_delete_deleter);  // NOTE: c2的析构函数不会被调用
