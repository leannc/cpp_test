...
void std_min_max_element_test() {
  ...
  auto target =
      std::min_element(std::begin(list), std::end(list), [](const Point &a, const Point &b) { return a.y < b.y; });
  ...
  auto max_target =
      std::max_element(std::begin(list), std::end(list), [](const Point &a, const Point &b) { return a.x < b.x; });
