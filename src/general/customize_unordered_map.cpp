...
namespace std {
    ...
    struct hash<CityRecord> {
        ...
        {
            return std::hash<std::string>()(cr.city) ^ std::hash<std::string>()(cr.country);
        }
    };
}
...
void customize_unordered_map()
{
    ...
    custom_map[CityRecord("Beijing","China",10000000,39.9,116.3)]=1; //NOTE:这里把key覆盖了，因为里面的参数与cr1是一样的
    custom_map[cr2]=2;
    custom_map[cr3]=3;
