...
TEST(AtmMachineTest, CanWithDrawOnCall) {
  ...
  ON_CALL(mock_server, Debit(_, _)).With(AllOf(Args<0, 1>(Lt()))).WillByDefault(Return(true));
  ...
  bool widthdraw_result4 = atm.Withdraw(
      account + 2000,
      amount);  // 不能过，因为Debit的条件没满足，account>amount了。虽然这个对比没有油盐，但是可以作为一个功能展示。
...
TEST(AtmMachineTest, UnExpectCall) {
  ...
  //---- 如果使用EXPECT_CALL的时候没有Times，也没有WillRepeatedly，那么默认是Times(1)，后面的调用都会失败。
  EXPECT_CALL(mock_server, GetBalance(345)).WillRepeatedly(Return(2000));  // 后面每次调用返回0

  EXPECT_CALL(mock_server, Debit(_, _)).WillRepeatedly(Return(true));

  EXPECT_CALL(mock_server, Connect).Times(AtLeast(1));

  EXPECT_CALL(mock_server, Disconnect).Times(AtLeast(1));

  ATMMachineAsTestTarget atm(&mock_server);
  bool widthdraw_result =
      atm.Withdraw(account, amount);  // 不能过，因为GetBalance的参数不对，它成了一个UnExpectCall，会直接导致fail。
...
TEST(AtmMachineTest, SequenceCall) {
  ...
  EXPECT_CALL(mock_server, Debit(_, _)).WillRepeatedly(Return(true)).InSequence(s1);

  EXPECT_CALL(mock_server, GetBalance(123)).WillRepeatedly(Return(2000)).InSequence(s1);

  ATMMachineAsTestTarget atm(&mock_server);
  bool widthdraw_result = atm.Withdraw(
      account, amount);  // 不能过，调用顺序指定的是先Debit再GetBalance，但是实际调用顺序是先GetBalance再Debit。
...
TEST(AtmMachineTest, SequenceCallAfter) {
  ...
  ATMMachineAsTestTarget atm(&mock_server);
  bool widthdraw_result =
      atm.Withdraw(account, amount);  // 不能过，调用顺序指定的是GetBalance要afterDebit，但是实际反的。
