...
auto invoke(F f) {
  if constexpr (std::is_void_v<decltype(f())>) {
    std::cout << "no return value" << std::endl;
      ...
      std::is_integral_v<std::invoke_result_t<
          F>>) {  /// 这个写法的作用，与上面的decltype(f())相同，只不过是换种写法，可以用于没有形参f的场景。
    std::cout << "int return value" << std::endl;
  } else if constexpr (
      std::is_same_v<
          decltype(std::declval<F>()()),
          std::
              string>) {  /// 第三种写法，会凭空搞一个默认构造函数，它不求值，只关心返回值类型，只有这里可以这样玩。这个是在没有形参f的情况下，使用的玩法
