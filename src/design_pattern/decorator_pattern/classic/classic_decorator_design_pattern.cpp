...
class Discounted : public DecoratedItem {
 public:
  explicit Discounted(double discountRate, std::unique_ptr<Item> item) :discount_rate(discountRate), DecoratedItem(std::move(item)) {}
...
class Taxed : public DecoratedItem {
 public:
  explicit Taxed(double taxRate, std::unique_ptr<Item> item) :tax_rate(1.0+taxRate), DecoratedItem(std::move(item)) {}
...
void classic_decorator_design_pattern() {
  std::unique_ptr<Item> item1(
      std::make_unique<Taxed>(0.13,
      std::make_unique<Discounted>(0.8,
      std::make_unique<Book>(10, "cpp"))));
