 ...
 *         (drawe函数)a. 改变ShapeModel的签名，再传入一个std::function，由这个std::function的成员变量，去完成具体的operation。这样动静很大，因为所有实例化ShapeModel的地方，都需要改
 *        (origin函数)b. 在concept里面，为新增的operation添一个参数，要求一个实现的函数，然后给个默认值，这样改动也比较小，只需要在model里面delegate一下就可以了，这样不关系的类，就用默认的函数，关心的类，自己写实现。
 *          (area函数)c. 直接调用ShapeT的成员函数，这样就会导致ShapeT再也不是简单的数据结构，而且跟普通的inheritance的工作量差不多。
