...
public:
    void operator()(Circle & c) {c.DrawCircle(); std::cout<<strategy<<std::endl;}
    void operator()(Box & b) {b.DrawBox();std::cout<<strategy<<std::endl;}
    void operator()(Square & s) {s.DrawSquare();std::cout<<strategy<<std::endl;}
    void operator()(auto & s) {s.DrawOther();std::cout<<strategy<<std::endl;}  //默认参数，如果没有匹配的，就调用这个
