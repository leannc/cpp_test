...
void cam_component_type_erasure_implementation() {
  auto mk_updater = [](MicroLink const &m) {
    std::cout << "micro_link p1:" << m.microlink_p1
              << ", p2:" << m.microlink_name << std::endl;
  };
  auto cp_updater = [](CoolPoint const &c) {
    std::cout << "cool_point p1 " << c.coolPoint_p1 << ", p2:" << c.coolPoint_p2
              << std::endl;
  };
  auto st_updater = [](StartPoint const &s) {
    std::cout << "start_point p1 " << s.start_p1 << std::endl;
  };
