...
void concepts_can_replace_inheritance() {
  ...
  //    printArea(Circle<int>{5});  ///报错，因为不满足 std::same_as<decltype(t.perimeter())>;
  //    这个返回值的条件，需要area()函数和perimeter()成员函数的返回值类型相同

  //    printArea(Rectangle<double>{5,2});  ///报错，因为不满足 std::is_base_of_v<ShapeBase,T>;
  //    需要是ShapeBase的子类才行
}
